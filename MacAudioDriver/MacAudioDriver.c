...
static SharedRingBuffer*        gRingBuffer = NULL;
...
static Float32                  gVolume_Input_Master = 1.0f;
static bool                     gMute_Input_Master = false;
static UInt32                   gDevice_BufferFrameSize = 512;
...
void* MacAudioDriver_Create(CFAllocatorRef allocator, CFUUIDRef requestedTypeUUID) {
    ...
    os_log_t createLog = os_log_create(kPlugIn_BundleID, "create");
    os_log(createLog, "MacAudioDriver_Create called");
    ...
    if (!CFEqual(requestedTypeUUID, audioPlugInTypeUUID)) {
        os_log_error(createLog, "MacAudioDriver_Create: UUID mismatch, returning NULL");
        CFRelease(audioPlugInTypeUUID);
        return NULL;
    }
    CFRelease(audioPlugInTypeUUID);

    os_log(createLog, "MacAudioDriver_Create returning driver ref %p", gDriverRef);
    return gDriverRef;
}
...
static HRESULT MacAudio_QueryInterface(void* inDriver, REFIID inUUID, LPVOID* outInterface) {
    ...
    if (CFEqual(interfaceUUID, iUnknownUUID) || CFEqual(interfaceUUID, plugInInterfaceUUID) || CFEqual(interfaceUUID, plugInInterfaceUUID2)) {
        os_log(qiLog, "QueryInterface: matched, returning driver ref");
    ...
    os_log(qiLog, "QueryInterface: no match, returning E_NOINTERFACE");
    CFRelease(interfaceUUID);
    CFRelease(plugInInterfaceUUID);
    CFRelease(plugInInterfaceUUID2);
    *outInterface = NULL;
...
static OSStatus MacAudio_Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost) {
    ...
    Float64 nanosPerFrame = 1000000000.0 / gDevice_SampleRate;
    gDevice_HostTicksPerFrame = nanosPerFrame / nanosPerTick;
    os_log(gLog, "Timebase configured: ticksPerFrame=%f", gDevice_HostTicksPerFrame);
    ...
    case kObjectID_Volume_Input_Master:
        switch (inAddress->mSelector) {
            ...
            case kAudioLevelControlPropertyScalarValue:
                ...
                *((Float32*)outData) = gVolume_Input_Master;
            ...
            case kAudioLevelControlPropertyDecibelValue:
                *outDataSize = sizeof(Float32);
                pthread_mutex_lock(&gDevice_IOMutex);
                *((Float32*)outData) = ScalarToDecibels(gVolume_Input_Master);
                pthread_mutex_unlock(&gDevice_IOMutex);
                return noErr;
    ...
    case kObjectID_Mute_Input_Master:
        switch (inAddress->mSelector) {
            ...
            case kAudioBooleanControlPropertyValue:
                *outDataSize = sizeof(UInt32);
                pthread_mutex_lock(&gDevice_IOMutex);
                *((UInt32*)outData) = gMute_Input_Master ? 1 : 0;
                pthread_mutex_unlock(&gDevice_IOMutex);
                return noErr;
        }
        break;
    ...
    switch (inObjectID) {
        case kObjectID_Device:
            if (inAddress->mSelector == kAudioDevicePropertyNominalSampleRate) {
                ...
                mach_timebase_info(&timebase);
                Float64 nanosPerTick = (Float64)timebase.numer / (Float64)timebase.denom;
                gDevice_HostTicksPerFrame = (1000000000.0 / gDevice_SampleRate) / nanosPerTick;
                pthread_mutex_unlock(&gDevice_IOMutex);
...
static OSStatus MacAudio_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID) {
    ...
    if (gDevice_IOClientCount == 0) {
        ...
        if (gRingBuffer) {
            SharedRingBuffer_Close(gRingBuffer);
            gRingBuffer = NULL;
        }
        gRingBuffer = SharedRingBuffer_CreateOrOpen(0);
        if (!gRingBuffer) {
            os_log(gLog, "StartIO: failed to open ring buffer");
        }
    }
...
static OSStatus MacAudio_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed) {
    ...
    Float64 elapsedTicks = (Float64)(currentHostTime - gDevice_AnchorHostTime);
    Float64 elapsedFrames = elapsedTicks / gDevice_HostTicksPerFrame;
    ...
    *outHostTime = gDevice_AnchorHostTime +
        (UInt64)((Float64)(cycleCount * kDevice_RingBufferFrames) * gDevice_HostTicksPerFrame);
    *outSeed = 1;
...
static OSStatus MacAudio_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer) {
    ...
    if (!gRingBuffer || !SharedRingBuffer_GetActive(gRingBuffer)) {
        memset(outBuffer, 0, totalSamples * sizeof(Float32));
        return noErr;
    }

    // Read from shared ring buffer using accessor function
    uint64_t framesToRead = SharedRingBuffer_Read(gRingBuffer, outBuffer, inIOBufferFrameSize);

    // Zero-fill any remaining frames if underrun
    if (framesToRead < inIOBufferFrameSize) {
        memset(&outBuffer[framesToRead * kNumChannels], 0,
               (inIOBufferFrameSize - (UInt32)framesToRead) * kNumChannels * sizeof(Float32));
    }

    // Apply volume and mute
    if (gMute_Input_Master) {
        memset(outBuffer, 0, totalSamples * sizeof(Float32));
    } else if (gVolume_Input_Master < 1.0f) {
        Float32 vol = gVolume_Input_Master;
